...
4. **Fresh Read-Only Sol Review Gate**:
   - Launches an isolated ephemeral reviewer dynamically inheriting the active Codex model (`gpt-6-astra`, `gpt-5.6-sol`, `gpt-5.5`, etc., with optional `-Model` / `-ReasoningEffort` overrides) without overriding `model_reasoning_effort` via bundled scripts (`scripts/run-fresh-reviewer.ps1` or `scripts/run-fresh-reviewer.sh`). The Windows reviewer runs from an empty disposable root with nonessential apps/plugins/browser/memory/skill integrations disabled and receives the complete bounded review bundle in its prompt, so it has no direct canonical-worktree dependency or remote plugin-catalog startup dependency.
